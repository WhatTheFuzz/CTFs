...
def enter_license():
    ...
    user_key = key_full_template_trial
...
def check_key(key, username_trial):
    ...
    if len(key) != len(key_full_template_trial):
        ...
        # Get the dynamic part of the key
        key_part_dynamic1_trial = ''
        for i in [4, 5, 3, 6, 2, 7, 1, 8]:
            key_part_dynamic1_trial += hashlib.sha256(username_trial).hexdigest()[i]

        # Combine everything to get the flag.
        key_full_template_trial = key_part_static1_trial + key_part_dynamic1_trial + key_part_static2_trial
...
def main():
    flag = check_key(key_full_template_trial, username_trial.encode())
